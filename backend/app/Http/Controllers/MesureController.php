...
class MesureController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($capteur->seuil_min !== null && $validated['valeur'] < $capteur->seuil_min) {
            ...
            \Log::info("Seuil MIN dépassé: {$validated['valeur']} < {$capteur->seuil_min}");
        } elseif ($capteur->seuil_max !== null && $validated['valeur'] > $capteur->seuil_max) {
            $alerteType = 'seuil_max';  // Type cohérent avec le frontend
            $valeurHorsSeuil = true;
            \Log::info("Seuil MAX dépassé: {$validated['valeur']} > {$capteur->seuil_max}");
        }
        ...
        if ($valeurHorsSeuil) {
            ...
            if ($alerteExistante) {
                ...
                \Log::warning("Nouvelle alerte créée", [
