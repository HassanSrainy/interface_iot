// src/components/cliniques/clinique-management-user.tsx
...
import useAuth from "../../hooks/useAuth"; // récupération user connecté
...
// Use cliniquesApi.Capteur shape as source of truth and extend lightly
...
export function CliniqueManagementUser(): React.ReactElement {
  ...
  // load when user changes (or on mount if already present)
  useEffect(() => {
    if (!user) {
  ...
  const reloadCliniques = async () => {
    ...
    try {
      let data: Clinique[] = [];
      if (user && (user as any).id && typeof cliniquesApi.getCliniquesByUser === "function") {
        data = await cliniquesApi.getCliniquesByUser((user as any).id);
      } else if (typeof cliniquesApi.getCliniques === "function") {
        // fallback (admin/dev)
        data = await cliniquesApi.getCliniques();
      } else {
        data = [];
      }
      setCliniques(data);
  ...
  const refreshAll = async () => {
    ...
    try {
      ...
      setCapteursMap({});
      let data: Clinique[] = [];
      if ((user as any).id && typeof cliniquesApi.getCliniquesByUser === "function") {
        data = await cliniquesApi.getCliniquesByUser((user as any).id);
      } else {
        data = await cliniquesApi.getCliniques();
      }
  ...
  // lazy loaders (unchanged, but they will load only floors/services for clinics we fetched)
  ...
  // *** IMPORTANT: only use cliniquesApi.getCapteursByService here (no sensorsApi) ***
  const loadCapteursForService = async (serviceId: number) => {
    if (!capteursMap[serviceId]) {
      let data: cliniquesApi.Capteur[] = [];

      if (typeof cliniquesApi.getCapteursByService === "function") {
        try {
          const resp = await cliniquesApi.getCapteursByService(serviceId);
          // Normalize lightly so we always have common fields expected by UI
          data = (resp || []).map((d: any) => ({
            ...d,
            matricule: d.matricule ?? d.nom ?? (d as any).identifiant ?? undefined,
            adresse_ip: d.adresse_ip ?? d.ip ?? undefined,
            adresse_mac: d.adresse_mac ?? d.mac ?? undefined,
          }));
        } catch (err) {
          console.error("Erreur getCapteursByService:", err);
          data = [];
        }
      } else {
        // fallback : aucune API disponible
        data = [];
      }
  ...
  // modal helpers (idem que ton code)
  ...
  // if no user show info card
  if (!user) {
    return h("div", { className: "p-6" },
      h(Card, null,
        h(CardHeader, null, h(CardTitle, null, "Cliniques")),
        h(CardContent, null, h("p", { className: "text-sm text-muted-foreground" }, "Aucun utilisateur connecté. Connectez-vous pour voir vos cliniques."))
      )
    );
  }
...
export default CliniqueManagementUser;
