...
export function CliniqueManagementUser(): React.ReactElement {
  ...
  if (!user) {
    return h("div", { className: "min-h-screen bg-slate-50 p-6 flex items-center justify-center" },
      h(Card, { className: "max-w-md w-full shadow-lg" },
        h(CardHeader, { className: "text-center" },
          h(Building2, { className: "w-16 h-16 mx-auto text-slate-300 mb-4" }),
          h(CardTitle, { className: "text-2xl" }, "Cliniques")
        ),
        h(CardContent, { className: "text-center" }, 
          h("p", { className: "text-slate-600" }, "Aucun utilisateur connecté."),
          h("p", { className: "text-sm text-slate-500 mt-2" }, "Connectez-vous pour voir vos cliniques.")
        )
      )
    );
  }
