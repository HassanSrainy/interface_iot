...
const formatDate = (date: Date, formatStr: string) => {
  ...
  return date.toLocaleDateString('fr-FR', options) + (formatStr.includes('HH:mm') ? ` ${date.toLocaleTimeString('fr-FR', { hour: '2-digit', minute: '2-digit', hour12: false })}` : '')
}
...
const generateHistoricalData = (capteur: CapteurWithRelations, days: number) => {
  ...
  for (let i = days * 24; i >= 0; i -= intervalHours) {
    ...
    data.push({
      time: formatDate(date, days <= 1 ? 'HH:mm' : days <= 7 ? 'dd/MM HH:mm' : 'dd/MM'),
      fullDate: date,
          ...
          <Popover open={showCustomCalendar} onOpenChange={setShowCustomCalendar}>
            <PopoverTrigger asChild>
              <Button variant="outline" className="w-48">
                ...
                {customDate ? formatDate(customDate, 'dd/MM/yyyy') : 'Sélectionner une date'}
              </Button>
            </PopoverTrigger>
